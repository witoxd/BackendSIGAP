...
import { AsignacionDocenteController } from "../controllers/asignacionDocente.controller"
import { Recurso, Accion } from "../types"

const router = Router()
const ctrl = new AsignacionDocenteController()
...
router.post(
  ...
  [
    ...
    body("materia").isString().notEmpty().withMessage("materia requerida"),
    body("horas_semanales").optional().isInt({ min: 1 }),
  ],
  validate,
...
router.put(
  ...
  [
    ...
    body("materia").optional().isString().notEmpty(),
    body("horas_semanales").optional().isInt({ min: 1 }),
  ],
  validate,
  checkPermission(Recurso.CURSOS, Accion.UPDATE),
