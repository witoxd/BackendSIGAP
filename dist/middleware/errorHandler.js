...
const errorHandler = (err, req, res, next) => {
    ...
    if (err instanceof AppError_1.AppError && err.isOperational) {
        return res.status(err.statusCode).json(Object.assign({ success: false, message: err.message, errors: err.errors }, (process.env.NODE_ENV === "development" && { stack: err.stack })));
    }
    ...
    return res.status(500).json(Object.assign({ success: false, message: "Ha ocurrido un error interno del servidor. Por favor, intente nuevamente.", code: "INTERNAL_SERVER_ERROR" }, (process.env.NODE_ENV === "development" && {
        error: err.message,
        stack: err.stack,
    })));
};
